...
program
  ...
  .option('--ejs-only [ejs-suffix]', '仅对后缀为 ejs 的文件进行模板内容替换')
  .option('--suffix [suffix]', '添加需要进行模板内容替换的文件后缀，如有多个使用英文逗号分隔')
  .option('--verbose')
  .action(async (source, target, options) => {
    const { data = '{}', suffix = '' } = options;
    await transformEjsDir(source, target, { ...options, suffix: suffix.split(','), data: JSON5.parse(data) });
  });
